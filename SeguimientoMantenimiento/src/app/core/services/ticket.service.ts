...
export class TicketService {
    ...
    return this.http
      ...
      .pipe(
        map((response) => ({
          ...
          origenTicket: typeof ticket.origenTicket === 'string' ? ticket.origenTicket : undefined,
  ...
  updateTicket(ticketId: string, body: ActualizarTicketRequestBody, fallback?: Ticket): Observable<Ticket> {
    return this.http.patch<ApiResponse<boolean>>(`${API_BASE_URL}/tickets/${ticketId}`, body).pipe(
      map(() => ({
        ...
        origenTicket: fallback?.origenTicket,
        causaRaiz: body.causaRaiz ?? fallback?.causaRaiz,
        solucionPropuesta: body.solucionPropuesta ?? fallback?.solucionPropuesta,
        esDesarrollo: body.esDesarrollo ?? fallback?.esDesarrollo,
        nombreHu: body.nombreHu ?? fallback?.nombreHu,
        historiaUsuario: body.nombreHu ?? fallback?.historiaUsuario,
        urlHu: body.urlHu ?? fallback?.urlHu,
        carpetaMedios: body.carpetaMedios ?? fallback?.carpetaMedios,
  ...
  private mapTicket(ticket: TicketDto): Ticket {
    ...
    return {
      ...
      nombreHu: ticket.nombreHu ?? undefined,
      historiaUsuario: ticket.nombreHu ?? undefined,
      urlHu: ticket.urlHu ?? undefined,
      carpetaMedios: ticket.carpetaMedios ?? undefined,
